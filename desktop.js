...
// Helper vars ----------------------------------
// Note on performance: do not set debug log to true in production. Also, keep the auditlog as 
// spares as possible.
var auditLog = false; // prints audit messages
var debugLog = false; //prints debug messages
...
for (var k = 0; k < input.length; k++){
    ...
    for (var l = 0; l < rdpPlatforms.length; l++){
        if (input[k].toLowerCase().indexOf(rdpPlatforms[l].toLowerCase())> -1){
            ...
            debug("candidate: " + input[k] + ", type: " + rdpPlatforms[l])
            break
         }
    }
}
